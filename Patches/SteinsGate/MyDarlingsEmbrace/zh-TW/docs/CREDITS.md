# 致謝 / Credits

本文件記錄《STEINS;GATE 比翼雙飛的戀人》台灣繁體中文化模組所依賴或參考的重要原作、工具、字型、AI 工具與社群貢獻來源。

## 原作

感謝 MAGES. Inc. 與所有參與 *STEINS;GATE* 系列開發與創作的人員。

Steam 國際版本由 **Spike Chunsoft Co., Ltd.** 發行。

本模組為非官方 fan localization；原作遊戲與所有相關權利仍屬各自權利人所有。完整聲明請參閱 [`../legal/NOTICE.md`](../legal/NOTICE.md)。

## MagesTools / EasyPatcher / MagesLib

感謝 **FENGberd** 開發並公開 [`fengberd/MagesTools`](https://github.com/fengberd/MagesTools)。

本專案的 MAGES 格式處理與 EasyPatcher 套用流程建立在該工具鏈之上；目前公開 Repository 是該專案的 fork，並加入台灣繁體中文 EasyPatcher 介面、Steam 安裝路徑偵測與本地化模組維護資料。

授權與第三方資訊請參閱 [`../legal/THIRD_PARTY_NOTICES.md`](../legal/THIRD_PARTY_NOTICES.md)。

## GenYo Gothic / 源樣黑體

感謝 **ButTaiwan** 維護 [`ButTaiwan/genyog-font`](https://github.com/ButTaiwan/genyog-font)。

本專案的繁中字型生成／維護流程使用 `GenYoGothic2TW-M.otf` 作為重要字型來源。GenYo Gothic 為基於 Adobe Source Han Sans / 思源黑體的開源字型家族，採 SIL Open Font License 1.1。

## OpenAI GPT-5.6 Sol

本專案的大量翻譯、重譯、台灣用語改寫、審查輔助、術語一致性分析與維護文件整理使用 **OpenAI GPT-5.6 Sol** 作為主要語言模型工具。

模型輸出仍受 JP-first source authority、terminology governance、independent review、Planner adjudication、runtime validation 與 playtest evidence 約束。詳細 AI 使用揭露請參閱 [`LOCALIZATION_METHODOLOGY.md`](LOCALIZATION_METHODOLOGY.md)。

## 官方繁體中文術語參考

本專案為維持 Science Adventure／STEINS;GATE 系列術語一致性，會參考既有官方繁體中文譯名；適用時，*STEINS;GATE ELITE* 的官方繁體中文術語是重要參考來源之一。

這類參考用於 terminology alignment，而不是將其他官方版本文本 wholesale 搬入本模組。

## 台灣 STEINS;GATE 玩家社群

感謝長期參與 *STEINS;GATE* 討論、作品推廣與譯名使用的台灣玩家社群。部分沒有單一適用官方譯名的網路文化、宅文化與系列慣用稱呼，會參考台灣社群長期形成的自然用語。

「社群慣用」不應取代可具名的直接 attribution。若後續確認特定公開 Wiki、翻譯專案、測試者或貢獻者對本模組形成可辨識的直接貢獻，應在此文件補上具名來源與連結。

## 測試、審查與問題回報

感謝所有參與翻譯校對、術語確認、blind review、runtime 測試、實機遊玩、顯示問題確認與問題回報的人員。

對於希望公開具名的個別貢獻者，可在取得適當名稱／帳號資訊後持續補充本文件。
